...
class BookmarkControllerTest {
	...
	void shouldBookmarks() throws Exception{
		// 1번 테스트 단순히 /api/bookmarks의 호출 여부 => 결과는 BookmarksDto
		mvc.perform(MockMvcRequestBuilders.get("/api/bookmarks"))
			.andExpect(status().isOk())
		// 2번 테스트 @BeforeEach에 의해서 모두 삭제되었기 때문에 total Element가 0이면 된다
	...
	// 페이징을 값을 입력해서 나오는 결과를 테스트한다
	// page가 1이 전달됐을 때 테스트, page가 2가 전달됐을 때 테스트
	@ParameterizedTest
